...
class ordenestalleres(models.Model):
    numerofolio= models.CharField(max_length=10, verbose_name='Numero de Folio', blank=True)
    ...
    serviciosolicita= models.CharField(max_length=50,choices=tipo, default='SSGG', blank=True, verbose_name='Servicio que Solicita')
    ####################################################################################
    estado = (('Pendiente','PENDIENTE'),('Terminada','TERMINADA'),('Abastecimiento','ABASTECIMIENTO'),('SDA','SDA'),('PROCESO','PROCESO'))
    estadoOT= models.CharField(max_length=20,choices=estado, default='', blank=True, verbose_name='Estado de la OT')
    ####################################################################################
    ...
    def __str__ (self):
        return "{0} ({1})".format(self.serviciosolicita, self.id )

    class Meta:
        verbose_name='Ordenestalleres de Trabajo SSGG'
        ordering = ['-id']