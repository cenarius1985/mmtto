...
class facturaroperia(models.Model):
    ...
    ####################################################################################
    estado = (('Pendiente','PENDIENTE'),('Pagada','PAGADA'),('Abastecimiento','ABASTECIMIENTO'),('SDA','SDA'),('Proceso','PROCESO'))
    estadofactura= models.CharField(max_length=20,choices=estado, default='', blank=True, verbose_name='Estado de Factura')
    ####################################################################################
    ...
    servicio=models.TextField(verbose_name='Servicio de Origen',choices=tipo)
...
####################################################################################
    ...
    def __str__ (self):
        return "{0} ({1})".format(self.proveedor, self.numerofactura )

    class Meta:
        verbose_name='Factura de Facturaroperia'
        ordering = ['-id']